...
from share4video.constants import DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_INDEX
from share4video.conversation import conv_templates
from share4video.mm_utils import (get_model_name_from_path, process_images,
                            tokenizer_image_token)
from share4video.model.builder import load_pretrained_model
from share4video.utils import disable_torch_init

from base import ViLLMBaseModel

class ShareGPT4Video(ViLLMBaseModel):
    def __init__(self, model_args):
        ...
        self.tokenizer, self.model, self.processor, self.context_len = load_pretrained_model(self.model_path, None, model_name, device_map='cpu')
        self.model = self.model.cuda(self.device).eval()
