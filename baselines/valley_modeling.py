...
from transformers import AutoTokenizer, SiglipImageProcessor, Qwen2VLImageProcessor, set_seed
import decord
...
class Valley:
    def __init__(self, model_path="bytedance-research/Valley-Eagle-7B", device="cuda"):
        self.device = device
        self.model = ValleyEagleChat(
    ...
    def generate(self, instruction, video_path):
        ...
        request = {
            "chat_history": [
                {'role': 'system', 'content': 'You are Valley, developed by ByteDance. You are a helpful assistant.'},
                {'role': 'user', 'content': instruction},
            ],
            "images": [transforms.ToPILImage()(image.permute(2, 0, 1)).convert("RGB") for image in video_frames],
        }
